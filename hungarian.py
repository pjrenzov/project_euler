...
class Hungarian:
    ...
    def calculate(self, input_matrix=None, is_profit_matrix=False):
        ...
        for index, row in enumerate(result_matrix):
            print("Row Min for row {} is  : {}".format(row,row.min()))
            result_matrix[index] -= row.min()

        print("Matrix after step 1 is : \n")
        printMatrix(result_matrix)

        # Step 2: Subtract column mins from each column.
        for index, column in enumerate(result_matrix.T):
            print("Column Min for column {} is  : {}".format(column,column.min()))
            result_matrix[:, index] -= column.min()
            
        print("Matrix after step 2 is : \n")
        printMatrix(result_matrix)
        ...
        print("Total covered rows and columns are : {}".format(total_covered),end="\n")
        print("Matrix after step 3 is : \n")
        printMatrix(result_matrix)
...
if __name__ == '__main__':
    ...
    hungarian = Hungarian(cost_matrix)
    hungarian.calculate()
    print("Minimum Cost:\t", hungarian.get_total_potential())  
    print("Optimum Assignment:\n\t", hungarian.get_results())
    print("-" * 80)
